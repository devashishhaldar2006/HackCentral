    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 my-8 overflow-hidden flex flex-col max-h-[90vh]"
        >
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <span className="material-symbols-outlined text-5xl text-yellow-400 mb-3">group_off</span>
                <h3 className="text-lg font-bold text-slate-700">No Participants Yet</h3>
                <p className="text-sm text-slate-500 mt-1">Check back later when users RSVP.</p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                {participants.map((p) => (
                  <div key={p._id} className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 border border-slate-200">
                    <img src={p.avatar} alt={p.fullName} className="w-12 h-12 rounded-full object-cover shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-900 truncate">
                        {p.fullName}
                        {p.teamName && (
                          <span className="ml-2 px-2 py-0.5 bg-yellow-100 text-yellow-900 text-[10px] uppercase font-black rounded-md">
                            Team: {p.teamName}
                          </span>
                        )}
                      </p>
                      ...
                      {(p.college || p.location) && (
                        <p className="text-[11px] text-slate-400 mt-1 flex items-center gap-2 truncate">
